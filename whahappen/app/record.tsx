...
export default function Record() {
  ...
  const start = async () => {
    ...
    timerRef.current = setInterval(() => {
      const sec = Math.min(MAX_SECONDS, Math.floor((Date.now() - t0) / 1000));
      setElapsed(sec);
    }, 250);
  ...
  return (
    <View style={{ flex: 1, backgroundColor: 'black' }}>
      ...
      {!!challengeText && (
        <Animated.View
          pointerEvents="none"
          style={{ position: 'absolute', left: 0, right: 0, top: 0, bottom: 0, alignItems: 'center', justifyContent: 'center', opacity: fade }}
        >
          <Text
            style={{
              width: '70%',
      ...
      {/* Countdown arriba-izquierda (alineado con cámara) */}
      {timeLeft > 0 && (
        <View style={{ position: 'absolute', top: TOP, left: 16, backgroundColor: 'rgba(0,0,0,0.45)', paddingHorizontal: 10, paddingVertical: 6, borderRadius: 999 }}>
          <Text style={{ color: 'white', fontWeight: '700' }}>⏳ {formatCountdown(timeLeft)}</Text>
        </View>
      )}

      {/* Botón flip arriba-derecha (misma altura) */}
      <TouchableOpacity
        onPress={() => setFacing(facing === 'back' ? 'front' : 'back')}
        style={{ position: 'absolute', top: TOP, right: 16, backgroundColor: 'rgba(0,0,0,0.45)', padding: 10, borderRadius: 999 }}
        activeOpacity={0.9}
      >
        <Ionicons name="camera-reverse-outline" size={24} color="#fff" />
      </TouchableOpacity>

      {/* Controles de grabación (más arriba) */}
